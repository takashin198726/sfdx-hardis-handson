...
It throws a `QueryException` if the provided value is null.
...
```mermaid
graph TD
  NotLikeConditionClause["NotLikeConditionClause"]:::mainApexClass
  click NotLikeConditionClause "/objects/NotLikeConditionClause/"
...
public with sharing class NotLikeConditionClause extends AbstractConditionClause {
  ...
  private String checkAndCast() {
    if (value == null) {
      throw new QueryException('Value cannot be null for NOT LIKE condition. field: ' + field);
    }
...
## Constructors
### `NotLikeConditionClause(sObjectType, fieldMap, field, value)`

Constructor for NotLikeConditionClause.

#### Signature
```apex
public NotLikeConditionClause(Schema.SObjectType sObjectType, Map<String,Schema.SObjectField> fieldMap, String field, Object value)
```
...
| field | String | The field name to apply the NotLIKE condition on. |
