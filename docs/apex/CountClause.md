...
SOQL `COUNT(fieldName)` aggregate function. 
 
This class validates that the specified field is of a data type that is supported 
by the `COUNT` function, preventing runtime query errors for unsupported types 
like Boolean.
...
```mermaid
graph TD
  CountClause["CountClause"]:::mainApexClass
  click CountClause "/objects/CountClause/"
...
## Constructors
### `CountClause(sObjectType, fieldMap, fieldName, alias)`

Constructor for CountClause.

#### Signature
```apex
public CountClause(Schema.SObjectType sObjectType, Map<String,Schema.SObjectField> fieldMap, String fieldName, String alias)
```
