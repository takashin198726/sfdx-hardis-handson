...
```mermaid
graph TD
  MinClause["MinClause"]:::mainApexClass
  click MinClause "/objects/MinClause/"
...
public with sharing class MinClause extends AbstractAggregateClause {
  ...
  public override IAggregateClause overrideMetaData(
    ...
    return new MinClause(sObjectType, fieldMap, fieldName, alias);
  }
  ...
  public override String build() {
    ...
    if (!supportedTypes.contains(type)) {
      String error = String.format(
        'The specified field type is not supported for MIN. object name: {0}, field name: {1}, field type: {2}',
        new List<String>{ sObjectType.getDescribe().getName(), fieldName, type.name() }
      );
      throw new QueryException(error);
    }

    return String.format('MIN({0}) {1}', new List<String>{ this.fieldName, this.alias });
  }
}
```
...
## Constructors
### `MinClause(sObjectType, fieldMap, fieldName, alias)`

Constructor for MinClause.

#### Signature
```apex
public MinClause(Schema.SObjectType sObjectType, Map<String,Schema.SObjectField> fieldMap, String fieldName, String alias)
```
