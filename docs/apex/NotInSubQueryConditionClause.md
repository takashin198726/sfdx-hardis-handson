...
# NotInSubQueryConditionClause Class

A concrete implementation of `AbstractConditionClause` that builds a 
SOQL `NOT IN` condition with a subquery. 
...
```mermaid
graph TD
  NotInSubQueryConditionClause["NotInSubQueryConditionClause"]:::mainApexClass
  click NotInSubQueryConditionClause "/objects/NotInSubQueryConditionClause/"
...
public with sharing class NotInSubQueryConditionClause extends AbstractConditionClause {
  ...
  /**
   * Constructor for NotInSubQueryConditionClause.
   *
   ...
   * @param field The field name in the main query to apply the not in subQuery condition on.
...
## Constructors
### `NotInSubQueryConditionClause(sObjectType, fieldMap, field, subQueryScribe)`

Constructor for NotInSubQueryConditionClause.

#### Signature
```apex
public NotInSubQueryConditionClause(Schema.SObjectType sObjectType, Map<String,Schema.SObjectField> fieldMap, String field, Scribe subQueryScribe)
```
...
| field | String | The field name in the main query to apply the not in subQuery condition on. |
