...
```mermaid
graph TD
LoggerSObjectHandler__mdt["Logger SObject Handler"]:::mainObject
click LoggerSObjectHandler__mdt "/objects/LoggerSObjectHandler__mdt/"
...
| :-------- | :---- | :--: | :---------- | 
| IsEnabled__c | Is Enabled | Checkbox | Controls if the SObject's trigger handler class should execute. This is useful for temporary disabling the logger's trigger handlers, but should typically be enabled. |
| SObjectHandlerApexClass__c | Trigger Handler Apex Class | Text | undefined |
| SObjectType__c | SObject Type | MetadataRelationship | undefined |
| SObjectTypeOverride__c | SObjectType Override | Text | Not all base platform types can be selected using the SObjectType picklist. If your object is not supported, supply the API name for the object here instead. |
